...
function shuffle(arr) {
  ...
  for (let i = arr.length - 1; i > 1; i--) {
    const j = Math.floor(Math.random() * i) //create a number is small than i
    let temp = arr[i]
    arr[i] = arr[j]
    arr[j] = temp
  }
...
export const getRandomColorPairs = (count) => {
  ...
  for (let i = 0; i < count; i++) {
    //randonColor function is provided from https://github.com/davidmerfield/randomColor
...
export function hinePlayAgainButton() {
  const playAgainButton = getPlayAgainButton()
